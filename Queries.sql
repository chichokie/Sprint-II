...
select nombre from persona where id in(select id_alumno from alumno_se_matricula_asignatura); 
--------------------------JOINS-----------------------------------------------------------------;
...
select g.nombre, a.tipo, count(a.creditos) from grado g inner join asignatura a on g.id=a.id_grado group by tipo;
